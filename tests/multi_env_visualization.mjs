...
console.assert(d.environmentIndex === -1, 'default index -1');
console.assert(d.incidentMedium === 'Air', 'design-level incident');

// 选环境 0
const e0 = resolveEnvironment(design, 0);
console.assert(e0.environmentIndex === 0, 'env0 index');
console.assert(e0.exitMedium === 'Water', 'env0 exit overridden');
console.assert(e0.substrate.material === 'BK7', 'env0 substrate');

// 越界回退设计级
const oob = resolveEnvironment(design, 99);
console.assert(oob.environmentIndex === -1, 'oob falls back');

// 选项与标签
const opts = environmentOptions(design);
console.assert(opts.length === 3 && opts[0].value === -1, 'options include design + 2 envs');
console.assert(environmentLabel(design.meritEnvironments[0]) === 'E1: Air → Water', 'label format E{n}: inc → exit');
