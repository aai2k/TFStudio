...
// Per-layer admittance arcs plus the air-side endpoint dot for one polarization.
...
// The three keyed markers: η_s (substrate start), Y₀ (final), η₀ (incident/AR target).
...
// Assemble every Plotly trace for the diagram (arcs + endpoint dots + keyed markers).
// colors bundles the theme palette so per-series helpers stay small.
function admittanceTraces(series, matColorMap, colors) {
...
function admittanceLayout(series, colors) {
