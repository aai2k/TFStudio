...
function riChartTraces(profile, quantity) {
    if (!profile) return [];
    const traces = [];
...
function riChartLayout(profile, quantity, matColorMap, colors) {
...
const riTotalMapX = (r, v) => r.start + (v / r.span) * r.w;

function riTotalTraces(placed, quantity) {
...
function riTotalShapes(placed, matColorMap, colors) {
    ...
    placed.forEach(r => {
        ...
        for (let k = 0; k < vl.length && k + 1 < bounds.length; k++) {
            ...
            shapes.push({
                type: 'rect',
                x0: riTotalMapX(r, bounds[k]), x1: riTotalMapX(r, bounds[k + 1]), xref: 'x',
                y0: 0, y1: 1, yref: 'paper',
        ...
        for (const b of bounds.slice(1, -1)) {
            shapes.push({
                type: 'line', x0: riTotalMapX(r, b), x1: riTotalMapX(r, b), xref: 'x',
                y0: 0, y1: 1, yref: 'paper',
                line: { color: gridColor, width: 1, dash: 'dot' },
            });
        }
...
function riTotalAnnotations(placed, textColor) {
    return placed.map(r => ({
...
function riTotalFigure(regions, quantity, matColorMap, colors) {
    ...
    const layout = {
        ...
        shapes: riTotalShapes(placed, matColorMap, colors),
        annotations: riTotalAnnotations(placed, textColor),
    };
    if (showBoth) {
        layout.yaxis2 = {
            title: { text: 'k', font: { color: '#ef5350', size: 12 } },
