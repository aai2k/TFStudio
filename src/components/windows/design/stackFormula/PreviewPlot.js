...
export function buildPreviewOption(data, c, referenceWavelength, lambdaAxis) {
    const series = [
...
export function PreviewPlot({ resolveMaterial, compiled, incidentId, substrateId, refLambda, c, height = 220, noPreviewText, lambdaAxis }) {
    ...
    useEffect(() => {
        const failed = data.errorKey || data.error;
        if (failed) disposeChart(divRef.current, chartRef);
        else drawChart(divRef.current, chartRef, buildPreviewOption(data, c, refLambda, lambdaAxis));
    });
    useChartTeardown(divRef, chartRef);
    return h('div', { style: { position: 'relative', width: '100%', height } },
        h('div', { ref: divRef, style: { width: '100%', height } }),
        (data.errorKey || data.error) && h('div', {
            style: {
