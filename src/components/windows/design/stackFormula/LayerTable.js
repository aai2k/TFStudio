...
function layerRow(l, i, refLambda, resolveMaterial, c) {
    const n = resolveMaterial(l.material).getNK(refLambda)[0];
...
export function LayerTable({ compiled, refLambda, resolveMaterial, c, sf }) {
    return h('div', { style: { maxHeight: 150, overflowY: 'auto', border: `1px solid ${c.border}`, borderRadius: 4 } },
        h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: 11.5, color: c.text } },
            ...
            h('tbody', {},
                compiled.ok ? compiled.layers.map((l, i) => layerRow(l, i, refLambda, resolveMaterial, c))
