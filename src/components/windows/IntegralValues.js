...
function OverlayChart({ spectrum, char, weighting, c, theme, minMaxMarks }) {
    ...
    const buildData = useCallback(() => {
        ...
        let weightVals = null, weightLabel = weighting?.label || '';
        if (weighting && spectrum.lambda) {
            const sampler = weighting.kind === 'photopic' ? null : weighting.sampler;
            if (sampler) {
                const raw = lam.map(l =>
                    (l >= weighting.lamMin && l <= weighting.lamMax) ? sampler(l) : 0);
                const mx = Math.max(...raw, 1e-30);
                weightVals = raw.map(v => 100 * v / mx);
            }
        }
        ...
        return { data, layout };
    }, [spectrum, char, weighting, charColor, bgColor, paperColor, gridColor, textColor, minMaxMarks]);
