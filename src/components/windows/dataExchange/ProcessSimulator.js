...
// ── Plot ──────────────────────────────────────────────────────────────────────
//
// The figure spec (traces + layout + config) is pure — it depends only on the
// curve arrays and theme colors, never on component state — so it lives at
// module scope. `colors` bundles the resolved theme palette; y-values are
// scaled to percent (0..1 → 0..100) to match the "%" axis.

function spectraColors(c) {
    return {
        bg:     c.bg     || '#1e1e1e',
...
function baselineTrace(lambdas, baseline, quantity, textColor, sp) {
    return {
...
function stepTraces(lambdas, stepCurves, currentStep, quantity, sp) {
...
function liveTrace(lambdas, liveCurve, quantity, accent, sp) {
    return {
...
function spectraTraces(data, colors, sp) {
    const { lambdas, baseline, stepCurves, liveCurve, currentStep, showSteps, quantity } = data;
    const traces = [];
    if (baseline) traces.push(baselineTrace(lambdas, baseline, quantity, colors.text, sp));
    if (showSteps && stepCurves) traces.push(...stepTraces(lambdas, stepCurves, currentStep, quantity, sp));
    if (liveCurve) traces.push(liveTrace(lambdas, liveCurve, quantity, colors.accent, sp));
    return traces;
}

function spectraLayout(quantity, colors) {
    return {
...
function SpectraChart({
    ...
    useEffect(() => {
        ...
        const traces = spectraTraces(
            { lambdas, baseline, stepCurves, liveCurve, currentStep, showSteps, quantity },
            colors, sp,
        );
        const layout = spectraLayout(quantity, colors);
        if (!initRef.current) {
    ...
    }, [c, lambdas, baseline, stepCurves, liveCurve, currentStep, showSteps, quantity, sp]);
