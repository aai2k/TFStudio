 ...
 * busy indicator) and the already-built spectrum `series`; everything else is
...
export function SimulationView({ p, set, c, B, run, N, layerIdx, frac, series, leftTop, playback }) {
    ...
    const barSeries = [{
        type: 'bar',
        data: [
            { value: tE, itemStyle: { color: '#e5484d' } },
            { value: tA, itemStyle: { color: '#19b3c4' } },
            { value: tT, itemStyle: { color: '#2da44e' } },
        ],
        animation: false,
    }];
        ...
        h(SplitPage, { c, leftWidth: 184,
            left: [
                ...
                h('div', { key: 'bars', style: { height: 150, flexShrink: 0 } },
                    h(Chart, {
                        series: barSeries, xCategories: [B.barE, B.barA, B.barT],
                        xTitle: '', yTitle: 'nm', c, minHeight: 0,
