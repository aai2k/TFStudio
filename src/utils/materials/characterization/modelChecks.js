/**
 * Danger signals read off a fitted model's own n(λ) and k(λ).
 *
 * The residual cannot catch any of these. Macleod's worked examples show a
 * model fitting its data perfectly while describing the wrong film, and the
 * shape of the fitted curve is what gives it away.
 *
...
/** The fitted model as n and k on an even grid across the fitted range. */
export function sampleFit(fit, rangeNm, samples = 200) {
 ...
 * infrared.
 */
export function extinctionOutOfRange(rows) {
...
export function indexOutOfRange(rows) {
    let lowest = Infinity;
