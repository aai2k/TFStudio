...
export function dominantWavelength(xy, white, observer) {
  ...
  if (Math.hypot(dx, dy) < 1e-9) return { dom: null, comp: null, purity: 0 };

  // Walk the locus segments; find where ray W + s·(d) (s>0) crosses.
  const hit = (vx, vy) => {
    let best = null;
...
export function ciede2000(l1, l2) {
  ...
  const h = (b, a) => { if (b === 0 && a === 0) return 0;
    let v = Math.atan2(b, a) * deg; return v < 0 ? v + 360 : v; };
