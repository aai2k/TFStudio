...
function finishCompare(qual, value, unit) {
    ...
    const fmt   = v => v == null || !Number.isFinite(v)
        ? '—'
        : isPct ? (v * 100).toFixed(3) + ' %'
        : unit === 'nm' ? v.toFixed(2) + ' nm'
        : String(v);
    ...
    let pass = false, deviation = 0, cmpStr = '';
    if (qual.cmp === 'ge') {
        pass = value >= qual.target;
        deviation = qual.target - value;   // > 0 means violated
        cmpStr = '≥ ' + fmt(qual.target);
    } else if (qual.cmp === 'le') {
        pass = value <= qual.target;
        deviation = value - qual.target;
        cmpStr = '≤ ' + fmt(qual.target);
    } else if (qual.cmp === 'eq') {
        deviation = Math.abs(value - qual.target);
        pass = deviation <= (qual.tol ?? 0);
        cmpStr = '= ' + fmt(qual.target) + ' ± ' + fmt(qual.tol);
    } else if (qual.cmp === 'between') {
        pass = value >= qual.lo && value <= qual.hi;
        deviation = value < qual.lo ? qual.lo - value
