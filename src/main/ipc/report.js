...
async function handleSaveHtml(ctx, html, suggestedName) {
  const { dialog, getMainWindow, app, fs, path, log, safeName } = ctx;
  try {
...
async function handleListPresets(ctx) {
  const { fs, path, log, reportPresetsDir } = ctx;
  try {
...
async function handleLoadPreset(ctx, name) {
  const { fs, path, log, reportPresetsDir, safeName } = ctx;
  try {
...
async function handleSavePreset(ctx, preset) {
...
async function handleDeletePreset(ctx, name) {
  const { fs, path, log, reportPresetsDir, safeName } = ctx;
  try {
    const base = String(name || '').replace(/\.tfsr$/i, '');
    const file = path.join(reportPresetsDir, safeName(base) + '.tfsr');
