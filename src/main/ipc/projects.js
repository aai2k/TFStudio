...
function register(ipcMain, ctx) {
  const { fs, path, log, dialog, getMainWindow, settingsPath, projectsDir, safeName, safeFilePath, writeFileAtomic } = ctx;
  ...
  ipcMain.handle('theme:import-vscode', async () => {
  ...
  ipcMain.handle('save-design', async (event, folderName, design) => {
    try {
  ...
  ipcMain.handle('import-tfs', async () => {
    const result = await dialog.showOpenDialog(getMainWindow(), {
  ...
  ipcMain.handle('rename-item', async (event, folderName, oldName, newName) => {
    try {
