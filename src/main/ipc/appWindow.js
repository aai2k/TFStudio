...
function register(ipcMain, ctx) {
  const { app, shell, fs, path, log, helpServer, getMainWindow, devToolsAllowed } = ctx;
  ...
  ipcMain.on('toggle-devtools', () => {
    if (!devToolsAllowed) return;   // DevTools disabled in packaged builds (unless --debug)
    const mainWindow = getMainWindow();
  ...
  ipcMain.handle('help:open', async (event, opts) => {
    try {
      ...
      log(`help:open failed: ${err.message}`);
  ...
  ipcMain.handle('app:dev-allowed', () => devToolsAllowed);
  ...
  ipcMain.on('diag:log', (event, msg) => { try { log(`[renderer] ${msg}`); } catch (_) {} });
}
