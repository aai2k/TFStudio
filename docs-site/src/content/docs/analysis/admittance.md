...
description: The optical admittance locus through the stack, and the design intuition it gives at a glance.
...
- A path that walks **toward `η₀`** (marked η₀) is matching the coating to the
  incident medium, which is an antireflection design. An endpoint sitting on
