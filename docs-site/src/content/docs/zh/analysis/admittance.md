...
在单一波长与单一入射角下，该图从基底导纳出发，依次施加每一层的传递。最终的端点就是前表面导纳 `Y`，由此可得反射率 `R = |(η₀ − Y) / (η₀ + Y)|²`，其中 `η₀` 为入射介质的导纳。
...
**面（Side）**：追踪**前（front）**镀膜（从入射介质出发）还是**后（back）**镀膜（从出射介质出发）。每一面的镀膜在基底上都是各自独立的轨迹，因此两侧是分别绘制的。
