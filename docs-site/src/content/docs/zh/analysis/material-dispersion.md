...
公式导数对存储系数精确。PCHIP 导数对穿过所提供表格的三次分段精确，但更高阶仍描述该插值选择。PCHIP 通过其第一导数连续；GDD 与 TOD 可能在表格节点处跳变。图在每个节点取样，并以穿过节点两侧数值的台阶画出跳变，因此曲线在此绝不断开。间隙的含义恰好相反，表示该样点没有数值：波长超出模型范围，或被厚度掩蔽。用户创建的平滑拟合仅在随材料存储的有效范围内使用。超出模型范围的点留空，窗口报告省略了多少个。
