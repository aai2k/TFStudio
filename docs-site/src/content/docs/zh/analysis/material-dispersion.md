...
公式导数对存储系数精确。PCHIP 导数对穿过所提供表格的三次分段精确，但更高阶仍描述该插值选择。PCHIP 通过其第一导数连续；GDD 与 TOD 可能在表格节点处跳变。图在每个节点取样，并以穿过节点两侧数值的台阶画出跳变，因此曲线在此绝不断开。间隙的含义恰好相反，表示该样点没有数值：被厚度掩蔽的波长，或其模型无法给出三阶导数的波长。窗口的提示会说明原因。

超出材料数据范围时曲线仍会绘制，并加上阴影带，注明材料及其实际覆盖的范围。表格在最后一行之外保持该行的数值，其折射率没有斜率，因此材料在那里不贡献任何色散：带内画出的是无色散平板的延迟。公式则在其拟合波段之外被外推，仍然具有色散，属于模型超出有效范围，而不是没有模型。用户创建的平滑拟合仅在随材料存储的有效范围内使用。结果表会用材料名标记这些行，导出的数值不会在无声中变成外推值；提示中也可一键把绘图范围收窄到已覆盖的跨度。
